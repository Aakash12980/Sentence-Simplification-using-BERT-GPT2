...
def compute_bleu_score(logits, labels):
    refs = Tokenizer.get_sent_tokens(labels, max_token_len)
...
def decode(**kwargs):
    ...
    for sent in sent_tensors:
        with torch.no_grad():
            print(f"input tokens: {sent[0]}")
            predicted = enc_dec_model.model.generate(sent[0].to(device), attention_mask=sent[1].to(device),
                             decoder_start_token_id=enc_dec_model.model.config.decoder.decoder_start_token_id)
            print(f'output tokens: {predicted.squeeze()}')
...
def train_model(start_epoch, eval_loss, loaders, optimizer, check_pt_path, best_model_path, model):
    ...
    for epoch in range(start_epoch, N_EPOCH):
        ...
        logging.info(f'Completed Epoch: {epoch} | avg. eval loss: {epoch_eval_loss:.5f} | blue score: {bleu_score} | time elapsed: {time.time() - eval_start_time}')
